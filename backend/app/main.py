...
def get_all_positions(minutes_from_now: float = 0.0, db: Session = Depends(get_db)):
    satellites = db.query(Satellite).all()
    return [
        {"id": sat.id, "name": sat.name, **propagate_satellite(sat.tle_line1, sat.tle_line2, sat.name, minutes_from_now)}
...
def get_groundtrack(id: int, steps: int = 18, step_minutes: float = 5.0, db: Session = Depends(get_db)):
    from fastapi import HTTPException
    sat = db.query(Satellite).filter(Satellite.id == id).first()
    if not sat:
        raise HTTPException(status_code=404, detail="Satellite not found")
...
@app.get("/cdm")
def get_conjunctions(threshold_km: float = 50.0, db: Session = Depends(get_db)):
    satellites = db.query(Satellite).all()
    if len(satellites) < 2:
        return []
    ...
    for i in range(len(current)):
        for j in range(i + 1, len(current)):
            ...
            # Step through next 90 minutes to find Time of Closest Approach
...
def get_sun_moon(minutes_from_now: float = 0.0):
    ...
    # Sun — low-precision solar coordinates (accuracy ~0.01°)
    ...
    # → Three.js scene (Y=up=north pole): ECI X→X, ECI Z→Y, ECI Y→-Z
    def to_scene(x, y, z):
...
def get_elements(id: int, db: Session = Depends(get_db)):
    from fastapi import HTTPException
    sat = db.query(Satellite).filter(Satellite.id == id).first()
    if not sat:
...
def delete_satellite(id: int, db: Session = Depends(get_db)):
    from fastapi import HTTPException
    sat = db.query(Satellite).filter(Satellite.id == id).first()
    if not sat:
        raise HTTPException(status_code=404, detail="Not found")
